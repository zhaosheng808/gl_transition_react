...
export class App extends Component{
    ...
    render(){
        ...
        for(let i = 0; i < countPage; i++){
            pagenumWrapper.push(<div className={currentPage == i ? 'activeButton button' : 'button'}  onClick={this.goPage.bind(null,i)} key={i}>{i+1}</div>)
        }
        if(loadImgNum >=2) {
        ...
        return (<div className="app_wrapper">
            ...
            <div className="time_box">
                ...
                请设置转场过渡时间：<input name="animateTime" onChange={this.setAnimateTime} type="number"/> s</div>
            <div className="card_wrapper">
                { loadImgNum>=2 ? currentAnimate.map((item, index) => {
                    return <CanvasCard imageFrom = {imageFrom} animateType={item} key={index} imageTo = {imageTo} cardIndex={index} animateTime ={animateTime} />
                }) : ''}
                <div className="clear"/>
            </div>
        </div>)
